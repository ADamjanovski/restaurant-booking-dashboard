...
const NewTable = () => {
  ...
  const formSubmitHandler = async (event) => {
    ...
    const data = await useHttp({
      method: "POST",
      url: "https://restaurant-booking-backend-damjanovski-phi.vercel.app/api/dashboard/tables",
      body: JSON.stringify({
  ...
  return (
    <div className="">
      <form className="flex flex-col my-4" onSubmit={formSubmitHandler}>
        <label htmlFor="tableNumber " className="mx-3 my-1 text-lg">
          Table number
        </label>
        <input
          onChange={tableChangeHandler}
          onBlur={tableInputBlurHandler}
          type="number"
          id="tableNumber"
