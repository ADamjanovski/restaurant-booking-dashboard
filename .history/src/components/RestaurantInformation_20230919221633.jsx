    ...
    <div className="w-full h-full ">
      <Card className="text-start ">
        <div className="flex flex-col">
          ...
          <p className="border-neutral-600 border-b py-2 px-2 mb-4">Categories</p>
          {categories &&
            categories.length > 0 &&
            categories.map((category) => (
              <div className="flex justify-between w-1/2 mb-3">
                <p className="py-2 px-2 w-24">{category.name}</p>
                <button className="bg-accent rounded-lg p-1">Delete category</button>
              </div>
            ))}
          ...
          <button
            onClick={handleOpen}
            className="bg-primary text-black w-1/2 self-center md:self-start rounded-lg text-black mb-2 p-1"
          >
            Add a category
          </button>
          {open && (
