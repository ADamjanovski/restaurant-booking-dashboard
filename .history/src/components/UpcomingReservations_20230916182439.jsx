...
const UpcomingReservations = () => {
  ...
  const AcceptHandler = ()=>{
    const acceptRequest = {
        url: `http://localhost:3000/api/dashboard/upcomingReservations/`,
        method: "GET",
        headers: {
          Authorization: `Bearer ${authCtx.token}`,
          Accept: "application/json",
                  ...
                  <tr key={reservation.id} className="pt-4">
                    ...
                    {reservation.status === "CONFIRMED" ? (
                      ...
                      <td className="text-center text-sm flex justify-around mt-4">
                        <button className="rounded-lg border bg-green-500 p-2 b">ACCEPT </button>
                        <button className="rounded-lg border bg-red-500 p-2 b">DECLINE</button>
                      </td>
                    )}
                  </tr>
                );
