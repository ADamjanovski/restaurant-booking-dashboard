...
const RestaurantInformation = () => {
  ...
  const deleteCategory = async (id) => {
    try {
      const response = useHttp({
        method: "DELETE",
        url: `https://restaurant-booking-backend-damjanovski-phi.vercel.app/api/dashboard/category/${id}`,
        headers: {
  ...
  useEffect(() => {
    useHttp({
      method: "GET",
      url: `https://restaurant-booking-backend-damjanovski-phi.vercel.app/api/dashboard/${authCtx.user.restaurantId}`,
      headers: {
        Authorization: `Bearer ${authCtx.token}`,
        ...
        <div className="flex flex-col">
          ...
          {categories &&
            ...
            categories.map((category) => (
              <div className="flex justify-between self-center md:self-start w-1/2 mb-3">
