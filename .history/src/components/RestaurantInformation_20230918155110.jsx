...
const RestaurantInformation = () => {
  ...
  useEffect(()=>{
    useHttp({
      method: "GET",
      url: `http://localhost:3000/api/dashboard/${authCtx.user.}`,
      body: JSON.stringify({
        tableNumber: tableInputValue,
      ...
      <Card className="text-start ">
        <div className="flex flex-col">
          ...
          {authCtx.user.categories &&
            ...
            authCtx.usercategories.map((category) => <p className="py-2 px-2">category.name</p>)}
          ...
          <button className="bg-accent w-1/2 self-center md:self-start rounded-lg text-black mb-2">Add a category</button>
        </div>
      </Card>
    </div>
  );
