...
const Images = () => {
  ...
  const getImages = () => {
    try {
      const data = useHttp({
        url: `https://restaurant-booking-backend-damjanovski.onrender.com/api/restaurants/images?restaurantId=${authCtx.user.restaurantId}`,
        method: "GET",
        headers: {
              ...
              <figure className="self-center" key={`figure ${image.id}`}>
                ...
                <div className="flex justify-around ">
                  ...
                  <button
                    ...
                    onClick={() => deleteImage(image.imageUrl, image.id)}
                    key={`button ${image.id}`}
                  >
                    Delete
                  </button>
